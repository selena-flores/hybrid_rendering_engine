...
int M3d_make_movement_sequence_matrix(double v[4][4], double vi[4][4], int n,
  int mtype[100], double mparam[100]){

    double temp[4][4],invtemp[4][4];
    ...
    for(int i = 0; i < n; i++){
      if(mtype[i] == SX){
        ...
        M3d_make_scaling(invtemp, 1/mparam[i], 1, 1);
      }
      else if(mtype[i] == SY){
        M3d_make_scaling(temp, 1, mparam[i], 1);
        M3d_make_scaling(invtemp, 1, 1/mparam[i], 1);
      }
      else if(mtype[i] == SZ){
        M3d_make_scaling(temp, 1,1,mparam[i]);
        M3d_make_scaling(invtemp, 1,1,1/mparam[i]);
      }
      else if(mtype[i] == RX){
        M3d_make_x_rotation_cs(temp, cos(conv*mparam[i]), sin(conv*mparam[i]));
        M3d_make_x_rotation_cs(invtemp, cos(-conv*mparam[i]), sin(-conv*mparam[i]));
      }
      else if(mtype[i] == RY){
        M3d_make_y_rotation_cs(temp, cos(conv*mparam[i]), sin(conv*mparam[i]));
        M3d_make_y_rotation_cs(invtemp, cos(-conv*mparam[i]), sin(-conv*mparam[i]));
      }
      else if(mtype[i] == RZ){
        M3d_make_z_rotation_cs(temp, cos(conv*mparam[i]), sin(conv*mparam[i]));
        M3d_make_z_rotation_cs(invtemp, cos(-conv*mparam[i]), sin(-conv*mparam[i]));
      }
      else if(mtype[i] == TX){
        M3d_make_translation(temp, mparam[i], 0, 0);
        M3d_make_translation(invtemp, -mparam[i], 0, 0);
      }
      else if(mtype[i] == TY){
        M3d_make_translation(temp, 0, mparam[i],0);
        M3d_make_translation(invtemp,0, -mparam[i], 0);
      }
      else if(mtype[i] == TZ){
        M3d_make_translation(temp, 0,0,mparam[i]);
        M3d_make_translation(invtemp, 0,0,-mparam[i]);
      }
      else if(mtype[i] == NX){
        M3d_make_scaling(temp, -1, 1,1);
        M3d_make_scaling(invtemp, -1, 1,1);
      }
      else if(mtype[i] == NY){
        M3d_make_scaling(temp, 1, -1,1);
        M3d_make_scaling(invtemp, 1, -1,1);
...
int M3d_view(double v[4][4], double vi[4][4], double eye[3],
  double coi[3], double up[3]){

    double temp[4][4];
    double tempi[4][4];

    M3d_make_identity(temp);
    M3d_make_identity(tempi);

    M3d_make_identity(v);
    M3d_make_identity(vi);

    double a,b,c, ai, bi, ci;
    ...
    double cs1 = c/p;//adjacent/hypoteneuse
    double s1 = -a/p;//opposite/hypoteneuse

    double cs2 = p/r;//adjacent/hyp
    double s2 = b/r;//op/hype

    M3d_make_translation(temp,-eye[0], -eye[1], -eye[2]);//move back to origin
    M3d_make_translation(tempi, eye[0], eye[1], eye[2]);
    M3d_mat_mult(v, temp, v);
    M3d_mat_mult(vi, vi, tempi);

    M3d_make_y_rotation_cs(temp, cs1, s1);//up over z axis
    M3d_make_y_rotation_cs(tempi, cs1, -s1);
    M3d_mat_mult(v, temp, v);
    M3d_mat_mult(vi, vi, tempi);

    M3d_make_x_rotation_cs(temp, cs2, s2);//up on z axis
    M3d_make_x_rotation_cs(tempi, cs2, -s2);
    M3d_mat_mult(v, temp, v);
    M3d_mat_mult(vi, vi, tempi);

    double upb[3] = {up[0], up[1],up[2]};
    double upc[3] = {up[0], up[1],up[2]};
    M3d_mat_mult_pt(upb, v, upb);//tranform from original position
    M3d_mat_mult_pt(upc, vi, upc);//tranform from original position
    a = upb[1];//adjacent
    b = upb[0];//opp
    ai = upc[1];//adjacent
    bi = upc[0];//opp
    c = sqrt(a*a + b*b);//hyp
    ci = sqrt(ai*ai + bi*bi);//hyp

    double cs3 = a/c;//adj/hyp
    double s3 = b/c;//opp/hype
    double cs3i = ai/ci;//adj/hyp
    double s3i = bi/ci;//opp/hype
    M3d_make_z_rotation_cs(temp, cs3, s3);//rotate up onto y axis
    M3d_make_z_rotation_cs(tempi, cs3, -s3);
    M3d_mat_mult(v, temp, v);
    M3d_mat_mult(vi, vi, tempi);
